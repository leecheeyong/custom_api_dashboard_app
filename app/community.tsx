...
export default function CommunityScreen() {
  ...
  const [communityWidgets, setCommunityWidgets] = useState<CommunityWidget[]>(DEFAULT_COMMUNITY_WIDGETS);
  const categories = ['All', 'Weather', 'Finance', 'Lifestyle', 'Developer', 'News', 'Entertainment'];
  ...
  const fetchCommunityWidgets = async () => {
    try {
      const res = await fetch('https://raw.githubusercontent.com/leecheeyong/community-widgets/refs/heads/main/widgets.json');
  ...
  const filteredWidgets = selectedCategory === 'All' 
    ? communityWidgets 
    : communityWidgets.filter(widget => widget.category === selectedCategory);
  ...
  const downloadWidget = async (widget: CommunityWidget) => {
    try {
      ...
      if (existingWidget) {
        Alert.alert('Already Installed', 'This widget is already in your dashboard');
        return;
      }
      ...
      Alert.alert(
        'Success',
        'Widget has been added to your dashboard!',
        [{ text: 'OK' }]
      );
    } catch (error) {
  ...
  return (
    <View style={[styles.container, isDarkMode && { backgroundColor: '#18181b' }]}> 
      <View style={[styles.header, isDarkMode && { backgroundColor: '#23232a', borderBottomColor: '#27272a' }]}> 
        <Text style={[styles.headerTitle, isDarkMode && { color: '#f1f5f9' }]}>Community Widgets</Text>
        ...
        {categories.map((category) => (
          <TouchableOpacity
            ...
            style={[
              ...
              selectedCategory === category && isDarkMode && { backgroundColor: '#3B82F6' }
            ]}
          ...
          >
            <Text
              style={[
                ...
                selectedCategory === category && isDarkMode && { color: 'white' }
              ]}
            >
              {category}
            </Text>
        ...
        {filteredWidgets.map((widget, index) => {
          ...
          return (
            <Animated.View
              ...
              style={[styles.widgetCard, isDarkMode && { backgroundColor: '#23232a', borderColor: '#27272a', shadowColor: '#18181b' }]
            }>
              <View style={styles.widgetHeader}>
                <View style={styles.widgetInfo}>
                  <Text style={[styles.widgetTitle, isDarkMode && { color: '#f1f5f9' }]}>{widget.title}</Text>
                  <Text style={[styles.widgetAuthor, isDarkMode && { color: '#a1a1aa' }]}>by {widget.author}</Text>
                </View>
                <View style={[styles.widgetCategory, isDarkMode && { backgroundColor: '#23232a' }]}> 
                  <Text style={[styles.categoryTag, isDarkMode && { color: '#a1a1aa' }]}>{widget.category}</Text>
                </View>
              </View>

              <Text style={[styles.widgetDescription, isDarkMode && { color: '#a1a1aa' }]}>{widget.description}</Text>
              ...
              <TouchableOpacity
                style={[
                  ...
                  isDarkMode && { backgroundColor: installed ? '#23232a' : '#3B82F6', borderColor: installed ? '#27272a' : '#3B82F6' }
                ]}
