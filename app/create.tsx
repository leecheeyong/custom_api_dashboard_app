...
export default function CreateWidgetScreen() {
  ...
  const editWidget = params.editWidget ? JSON.parse(params.editWidget as string) : null;
  ...
  const [fieldType, setFieldType] = useState<'text' | 'number' | 'boolean'>('text');
  ...
  const addField = () => {
    ...
    setFields([...fields, { id: Date.now().toString(), label: fieldLabel, key: fieldKey, type: fieldType }]);
    setFieldLabel('');
    setFieldKey('');
    setFieldType('text');
  };
  ...
  return (
    ...
    >
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.section, isDarkMode && { backgroundColor: '#23232a' }]}> 
          <Text style={[styles.sectionTitle, isDarkMode && { color: '#f1f5f9' }]}>Widget Details</Text>
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isDarkMode && { color: '#a1a1aa' }]}>Title *</Text>
            <TextInput
              style={[styles.input, isDarkMode && { backgroundColor: '#23232a', color: '#f1f5f9', borderColor: '#27272a' }]}
              value={title}
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isDarkMode && { color: '#a1a1aa' }]}>API URL *</Text>
            <TextInput
              style={[styles.input, isDarkMode && { backgroundColor: '#23232a', color: '#f1f5f9', borderColor: '#27272a' }]}
              value={apiUrl}
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.label, isDarkMode && { color: '#a1a1aa' }]}>Refresh Interval (seconds)</Text>
            <TextInput
              style={[styles.input, isDarkMode && { backgroundColor: '#23232a', color: '#f1f5f9', borderColor: '#27272a' }]}
          ...
          <TouchableOpacity
            style={[styles.testButton, isDarkMode && { backgroundColor: '#3B82F6' }]}
            onPress={testApiEndpoint}
            disabled={testing}
          >
            <Text style={[styles.testButtonText, isDarkMode && { color: 'white' }]}> 
              {testing ? 'Testing...' : 'Test API'}
            </Text>
          </TouchableOpacity>
          {testData && (
            <View style={[styles.testResult, isDarkMode && { backgroundColor: '#18181b' }]}> 
              <Text style={[styles.testResultTitle, isDarkMode && { color: '#f1f5f9' }]}>Test Result:</Text>
              <Text style={[styles.testResultData, isDarkMode && { color: '#a1a1aa' }]}> 
        ...
        <View style={[styles.section, isDarkMode && { backgroundColor: '#23232a' }]}> 
          <Text style={[styles.sectionTitle, isDarkMode && { color: '#f1f5f9' }]}>Appearance</Text>
          <Text style={[styles.label, isDarkMode && { color: '#a1a1aa' }]}>Color Presets</Text>
            ...
            {colorPresets.map((preset, index) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  backgroundColor === preset.background && styles.colorPresetSelected,
              ...
              >
                <Text style={[
                  ...
                  { color: (!isDarkMode || !isLightColor(preset.background)) ? preset.text : '#18181b' }
                ]}>
                  {preset.name}
                </Text>
              </TouchableOpacity>
        ...
        <View style={[styles.section, isDarkMode && { backgroundColor: '#23232a' }]}> 
          <Text style={[styles.sectionTitle, isDarkMode && { color: '#f1f5f9' }]}>Custom Fields</Text>
          <View style={styles.customFieldsRow}>
            <TextInput
              style={[styles.input, { flex: 1, marginRight: 8 }, isDarkMode && { backgroundColor: '#23232a', color: '#f1f5f9', borderColor: '#27272a' }]}
              value={fieldLabel}
            ...
            <TextInput
              ...
              placeholderTextColor={isDarkMode ? '#52525b' : '#94a3b8'}
            />
            <TouchableOpacity onPress={addField} style={[styles.addFieldButton, isDarkMode && { backgroundColor: '#3B82F6' }]}> 
              <Text style={[styles.addFieldButtonText, isDarkMode && { color: 'white' }]}>＋</Text>
            </TouchableOpacity>
          </View>
            ...
            {['text', 'number', 'boolean'].map(type => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  fieldType === type && isDarkMode && { backgroundColor: '#3B82F6' }
                ]}
                onPress={() => setFieldType(type as any)}
              >
                <Text style={fieldType === type ? { color: 'white' } : isDarkMode ? { color: '#a1a1aa' } : { color: '#1e293b' }}>{type}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <View style={styles.fieldsList}>
            {fields.length > 0 && fields.map((field, idx) => (
              <View key={field.id || field.label || idx} style={[styles.fieldPill, isDarkMode && { backgroundColor: '#18181b' }]}> 
                <Text style={[styles.fieldPillText, isDarkMode && { color: '#f1f5f9' }]}>{field.label} ({field.key}) [{field.type}]</Text>
                <TouchableOpacity onPress={() => removeField(field.id)}>
                  <Text style={[styles.fieldPillRemove, isDarkMode && { color: '#ef4444' }]}>×</Text>
                </TouchableOpacity>
              </View>
            ))}
          </View>
        </View>
        <TouchableOpacity style={[styles.saveButton, isDarkMode && { backgroundColor: '#3B82F6' }]} onPress={saveWidget}>
          <Text style={[styles.saveButtonText, isDarkMode && { color: 'white' }]}> 
